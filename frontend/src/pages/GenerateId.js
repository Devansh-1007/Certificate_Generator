...
const GenerateId = () => {
  ...
  const submit = async (e) => {
    ...
    setResult(null);
    try {
      const { data } = await api.post("/generateId", { ID_NAME: name });
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12">
      ...
      <p className="mb-8 text-sm text-slate-400">Creates an ID card with an embedded QR code, exported as PNG + PDF.</p>
