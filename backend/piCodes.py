...
def generateBar(Client, TABLE):
    img_url = getIDInfo(
        'ID_IMG_PATH', Client.CLIENT_ID, TABLE)
    pdf_url = getIDInfo(
        'ID_PDF_PATH', Client.CLIENT_ID, TABLE)
    ...
    try:
        ...
        # Log success message
        logging.info(
            'Barcode generated for Client: {}'.format(Client.CLIENT_ID))

        return barcode_data

    except Exception as e:
        # Log error message
        logging.error('Failed to generate barcode for Client: {}. Error: {}'.format(
