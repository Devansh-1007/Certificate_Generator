...
def loginClient(CLIENT_ID, CLIENT_NAME, PASSWORD):
    try:
        if verify_Client(CLIENT_ID, CLIENT_NAME, PASSWORD, "CLIENT_DETAILS"):
            ...
            SECRET_KEY = getClientInfo("CLIENT_SECRET_KEY", CLIENT_ID, "CLIENT_DETAILS")
            if SECRET_KEY is None:
                return jsonify({"error": "Login failed"}), 500
            IV = getClientInfo("CLIENT_IV", CLIENT_ID, "CLIENT_DETAILS")
            if IV is None:
                return jsonify({"error": "Login failed"}), 500
        ...
        logging.info("yaha atak chuka hoon")
