...
def register():
    ...
    if response == True and error_code == 200:
        ...
        client = Client(
            data["CLIENT_ID"], data["CLIENT_NAME"], hash_password, "NULL", "NULL"
        )
        try:
        ...
        logging.info(
            {
                "description": "Admin role required for this request.Token not verified.",
                "Error:%s": error_code,
            }
        )
        return (
        ...
        try:
            ...
            if checkCert:
                ...
                try:
                    ...
                    client.CERTIFICATE.CERTIFICATE_IMG_PATH = result[
                        "CERTIFICATE_DETAILS"
                    ]["IMAGE_URL"]
                    client.CERTIFICATE.CERTIFICATE_PDF_PATH = result[
                        "CERTIFICATE_DETAILS"
                    ]["PDF_URL"]
                    client.CERTIFICATE.CERTIFICATE_BASE64 = result[
                        "CERTIFICATE_DETAILS"
                    ]["BASE64"]
                except Exception as error:
                    logging.info("ab mei idhar aa gya hoon lekin kyu")
                    return str(error), 500

                try:
                    if insertCertificate(
                        client.CERTIFICATE, client, "CERTIFICATE_DETAILS"
                    ):
                        ...
                        return (
                            jsonify(
                                {
                                    "description": "Data cannot be inserted into the database"
                                }
                            ),
                            401,
                        )
...
def generateId():
    try:
        ...
        try:
            checkCert = verify_ID(ID_NAME, CURRENT_CLIENT, "ID_DETAILS")
            if checkCert:
                details = {
                    "status": "Success",
...
def getId():
    ...
    try:
        if EXTENSION == "pdf":
            try:
                downloadFile(
                    bucket, allIdPdfPath + "/" + CURRENT_CLIENT, str(ID_NAME) + ".pdf"
                )
            ...
            try:
                downloadFile(
                    bucket, allIdImgPath + "/" + CURRENT_CLIENT, str(ID_NAME) + ".png"
                )
...
def getAllCert():
    ...
    try:
        ...
        return (
            jsonify({"description": "Failed to retrieve files", "error": str(error)}),
            500,
        )
