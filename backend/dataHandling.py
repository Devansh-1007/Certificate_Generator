...
def insertClient(Client, TABLE):
    if request.method == "POST":
        ...
        values = (Client.CLIENT_ID, Client.PASSWORD, Client.CLIENT_NAME, admin, KEY, IV)

        try:
            cursor.execute(sql, values)
            mydb.commit()
            logging.info(
                "Data inserted into table '%s' for Client with ID '%s'",
                TABLE,
                Client.CLIENT_ID,
            )
            return True
            ...
            logging.error(
                "Failed to insert data into table '%s' for Client with ID '%s'. Error: %s",
                TABLE,
                Client.CLIENT_ID,
                str(e),
            )
...
def updateClient(COLUMN, newValue, Client, TABLE):
    if request.method == "POST":
        ...
        sql = (
            ...
            + " SET "
            + COLUMN
            + " = %s , `UPDATED_ON` = NOW() WHERE `CLIENT_ID` = %s"
        )
        try:
            cursor.execute(sql, (newValue, Client.CLIENT_ID))
            mydb.commit()
            logging.info(
                "Data updated in table '%s' for Client with ID '%s'",
                TABLE,
                Client.CLIENT_ID,
            )
        except Exception as e:
            logging.error(
                "Failed to update data in table '%s' for Client with ID '%s'. Error: %s",
                TABLE,
                Client.CLIENT_ID,
                str(e),
            )
...
def insertCertificate(CERTIFICATE, Client, TABLE):
    if request.method == "POST":
        ...
        values = (
            ...
            CERTIFICATE.CERTIFICATE_IMG_PATH,
            Client.CLIENT_ID,
            Client.CLIENT_ID,
        ...
        try:
            ...
            mydb.commit()
            logging.info(
                "Data inserted in table '%s' for Client with ID '%s'",
                TABLE,
                Client.CLIENT_ID,
            )
            return True
